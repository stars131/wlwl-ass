try:
    ...
    # 不在 import 时硬抛——允许仅做 import 不调用 web 工具的场景。
    # 实际用到时再 raise 一个明确的 RuntimeError，比 NameError 友好。
    _bs4_err_msg = str(_bs4_err)
    def BeautifulSoup(*_a, **_kw):
        raise RuntimeError(
            f"BeautifulSoup4 未安装：{_bs4_err_msg}。请 `pip install beautifulsoup4` 后再使用 web 相关工具。"
        )
    print(f"[Error] BeautifulSoup4 未安装（{_bs4_err_msg}）；web 工具将不可用，需要时请安装 beautifulsoup4。")
