...
CLAUDE_CONFIG_KEY = 'claude_config141'   # config_store providers 段里的条目名
OPENAI_CONFIG_KEY = 'oai_config1'        # config_store providers 段里的条目名
MODELSCOPE_API_KEY = ''                  # 直接填你的 ModelScope token
DEFAULT_BACKEND = 'claude'               # 默认后端: 'claude' / 'openai' / 'modelscope'
# =================================================================
#
# 通过 GUI 的 API 配置 tab 或 `python -m launcher.config set providers.<name>.api_key sk-...`
# 维护凭据；本文件只引用名称。条目应有 api_key / base_url / model 字段，
# 'claude' 后端约定为 Anthropic-shaped 端点 (apibase + /v1/messages)。
...
def ask_vision(image_input, prompt="详细描述这张图片的内容", timeout=60, max_pixels=1440000, backend=DEFAULT_BACKEND):
    ...
    try:
        if backend == 'claude':
            ...
            cfg = providers.get(OPENAI_CONFIG_KEY) or {}
            return _call_openai_compat(
                b64, prompt, timeout,
                apibase=cfg.get('base_url') or cfg.get('apibase'),
...
def _load_providers():
    """Read the providers section from launcher.config_store.

    Returns ``{name: {api_key, base_url, model, ...}}``. Resolves
    keyring-backed values; safe to call multiple times.
    """
    from launcher.config_store import default_store
    return dict(default_store().all().get('providers', {}))

def _call_claude(b64, prompt, timeout, max_tokens=1024):
    providers = _load_providers()
    cfg = providers.get(CLAUDE_CONFIG_KEY) or {}
    apibase = cfg.get('base_url') or cfg.get('apibase')
    apikey = cfg.get('api_key') or cfg.get('apikey')
    model = cfg.get('model')
    if not apibase or not apikey or not model:
        raise KeyError(f"providers.{CLAUDE_CONFIG_KEY} missing api_key/base_url/model")
    resp = requests.post(
        apibase + '/v1/messages',
