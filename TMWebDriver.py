...
class TMWebDriver:  
    def __init__(self, host: str = '127.0.0.1', port: int = 18765):
        ...
        # 检测端口是否被远端占用；用 with 关闭探测 socket，避免 fd 泄漏
        ...
        def long_poll():
            ...
            while time.time() - start_time < 5:
                try:
                    ...
                    except (ValueError, TypeError): traceback.print_exc()  # JSON 解析失败而已
                    return msg
    ...
    def set_session(self, url_pattern: str) -> Optional[str]:
        ...
        if not matched:
            ...
            return None
        if len(matched) > 1:
            print(f"警告: 找到多个URL包含 '{url_pattern}' 的会话，选择第一个")
