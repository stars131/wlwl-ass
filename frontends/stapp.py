...
def init():
    ...
    # ADR-0006: prefer WLWL_LLM_CONFIG_NAME (stable name) over WLWL_LLM_NO (index).
    config_name = os.environ.get("WLWL_LLM_CONFIG_NAME", "").strip()
    try:
        if config_name and agent.select_llm_by_name(config_name):
            pass  # selected by name
        else:
            agent.next_llm(int(os.environ.get("WLWL_LLM_NO", "0") or 0))
    except Exception:
        pass
    permission_mode = os.environ.get("WLWL_PERMISSION_MODE", "auto") or "auto"
    project_root = os.environ.get("WLWL_PROJECT_ROOT", "").strip() or None
    use_project_context = os.environ.get("WLWL_USE_PROJECT_CONTEXT", "1").strip().lower() not in {"0", "false", "no", "off"}
    agent.configure_cli(
    ...
    if agent.llmclient is None:
        st.error("⚠️ 未配置任何可用的 LLM 接口，请设置 .env 或运行 `python -m launcher.cli_init`。")
        st.stop()
...
_proj_name = os.environ.get("WLWL_PROJECT_NAME", "").strip()
_proj_id = os.environ.get("WLWL_PROJECT_ID", "").strip() or "default"
st.title(f"🖥️ Cowork · {_proj_name}" if _proj_name else "🖥️ Cowork")

if 'autonomous_enabled' not in st.session_state:
    st.session_state.autonomous_enabled = os.environ.get("WLWL_AUTONOMOUS_ENABLED", "0").strip().lower() in {"1", "true", "yes", "on"}
