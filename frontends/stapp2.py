...
def init():
    ...
    if agent.llmclient is None:
        st.error("⚠️ 未配置任何可用的 LLM 接口，请设置 .env、运行 `python -m launcher.cli_init`，或在 GUI 的 API 配置 tab 里新增一个。")
        st.stop()
    else:
...
(() => {
    ...
    if (hostWin.__wlwlAssHeaderBadgeTimer) {
        hostWin.clearInterval(hostWin.__wlwlAssHeaderBadgeTimer);
    }
    hostWin.__wlwlAssHeaderBadgeTimer = hostWin.setInterval(ensureBadge, 500);
...
def init_session_state():
    for key, value in {
        'agent_name': 'wlwl-ass', 'streaming': False, 'stopping': False, 'display_queue': None,
